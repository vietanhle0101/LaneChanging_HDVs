...
function set_state(c::MPC, CAV::Car, front_HDV::Car, behind_HDV::Car)
    c.st = CAV.st
    c.u = CAV.u
...
function nonlinearMPC(c::MPC, vd_H, CFM_params; solver = "Ipopt")
    ...
    @constraint(model, zH[:,1] .== [c.st_Hb[1], c.st_Hb[4]])
    # Bound constraints
    ...
    @constraints(model, begin
        c.params["y_min"] - cutoff .<= z[2,:] .<= c.params["y_max"] + cutoff
    end)
    ...
    # Headway constraint
    @constraint(model, z[1,2] + CFM_params["τs"]*z[4,2] + CFM_params["ds"] <= (c.st_Hf[1] + c.st_Hf[4]*dt))

    # Objective function
    J = sum(c.params["Wu"].*u.^2) + sum(c.params["Wv"]*(z[4,:] .- vd).^2) + sum(c.params["Wy"]*(z[2,:] .- y_ref).^2)
        + sum(c.params["WΔu"].*(u[:,1]-c.u).^2) + sum(c.params["WΔu"].*(u[:,2:end]-u[:,1:end-1]).^2) 
    
    J += sum(c.params["WHu"].*uH.^2) + sum(c.params["WHv"]*(zH[2,:] .- vd_H).^2)
